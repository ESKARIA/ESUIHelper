...
public struct Maker {
    ...
    public func headerView(title: String? = nil, details: String? = nil) -> ESHeaderView {

        let header = ESHeaderView()
    ...
    public func buttonWithImage(icon: UIImage?, position: ImageInButtonPosition, iconColor: UIColor, size: CGSize, buttonHeigh: CGFloat? = nil, iconOffset: CGFloat? = nil) -> UIButton {
        ...
        _ = button.heightAnchor.constraint(equalToConstant: height)
    ...
    public func radialButton(title: String) -> UIButton {
        ...
        let buttonSignin = ESUIBuilder.make.roundButtonWith(title: title, fontSize: 17, height: 34, arrow: false, textAlighnment: .center, whiteBackground: false, mainColor: UIColor.RGB(r: 56, g: 209, b: 133))
    ...
    public func iconLabel(image: UIImage = UIImage(), text: String?, insets: UIEdgeInsets = UIEdgeInsets.zero, uppercased: Bool) -> ESIconLabelView {
        let modifiedText = uppercased ? text?.uppercased() : text
        return ESIconLabelView(icon: image, text: modifiedText, insets: insets)
    }

}

public struct ESUIBuilder {
